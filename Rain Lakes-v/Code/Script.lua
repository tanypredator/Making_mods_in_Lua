...
function RainLake:GetWaterLevelPct()
...
function OnMsg.TerraformThresholdPassed(id, reached)
  if id == "LiquidWater" then
    local objs = MapGet("map", "RainLake")
    for i = 1, #objs do
    objs[i]:AddVolume(10)
    end
  end
...
local idx = table.find(building, "RainLake_Adjust", true)
if idx then
   if not type(building[idx]) == "function" then
       building[idx]:delete()
   end
...
PlaceObj("XTemplate", {
  ...
  PlaceObj("XTemplateTemplate", {
    ...
    "RolloverText",    T(11938, "<left>Water Level <right><percent(WaterLevelPct)>"),
    "RolloverTitle",    T(49, "Status"),
    "Title",    T(49, "Status"),
...
local constr = XTemplates.ipConstruction[1]
local idx1 = table.find(constr, "Elevation", true)
if idx1 then
   if not type(constr[idx1]) == "function" then
       constr[idx1]:delete()
   end
   table.remove(constr, idx1)
end
	...
	table.insert(
		...
		PlaceObj('XTemplateTemplate', {
			"Elevation", true,
			"comment", "terrain elevation",
			"__context_of_kind", "RainLake",
			"__template", "InfopanelSection",
			"Icon", "UI/Icons/Sections/construction.tga",
			"RolloverText", T(905, [[Terrain elevation]]),
			'Title', T(904, [[Terrain]])},

				{PlaceObj('XTemplateTemplate', {
				'__template', "InfopanelText",
				'Text', T(906, [[Elevation<right><RainLakeElevation>]])
			})
			})
	)
